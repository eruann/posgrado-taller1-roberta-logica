...
3. 'centering' - Same as per_type (alias for clarity)
4. 'standard' - Standard scaling (mean=0, std=1) per vector type
5. 'all_but_mean' - Original aggressive global approach
...
Usage:
    python scripts/run_normalization_comparison.py \\
        ...
        --output_dir data/snli/normalization_comparison \\
        --layer_num 9
"""
...
def run_command(cmd: list[str], allow_failure=False):
    ...
    try:
        ...
        if allow_failure:
            print(f"⚠️  COMMAND FAILED (continuing): {' '.join(cmd[:3])}...")
...
def main():
    ...
    normalization_types = ['none', 'per_type', 'standard']  # Removed problematic all_but_mean
    ...
    for norm_type in normalization_types:
        ...
        if norm_type == 'none':
            ...
            if not norm_file.exists():
                ...
                if not success:
                    print(f"❌ SKIPPED: {norm_type} normalization failed")
                    continue
        ...
        for n_comp in pca_components:
            ...
            for reduction_type in ["pca", "zca"]:
                pca_variant_file = pca_dir / f"{reduction_type}_{pca_file.name}"
                if not pca_variant_file.exists():
                    continue
