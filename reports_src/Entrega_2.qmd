---
title: "Taller de Tesis I – Entrega II"
...
format:
  pdf:
    ...
    toc: false
execute:
  echo: false        # oculta el código salvo que lo cambies
  warning: false     # oculta advertencias
  message: false     # oculta mensajes de librerías
---

## 1 · Pregunta y objetivos revisados {#pregunta}
...
Se usara Vscode + Quarto. Quarto es una herramienta *open-source* que convierte documentos que combinan Markdown + código en pdf. Como alternativa se usara Vscode + LaTex si se descubre que el trabajo en Quarto trae alguna complejidad o problemas.
...
#. Se genera un *embedding* por enunciado y se combinan (concatenación y diferencia) las representaciones vectoriales de premisa e hipótesis de modo que captemos tanto las características individuales como las discrepancias semánticas. 
#. Se aplica **PCA** para reducir la dimensionalidad de 768 dimensiones a un numero menor, 50 dimensiones por ejemplo, a fin de poder compactar el espacio vectorial y reducir coste computacional 
#. Se aplica **UMAP** para proyectar en dos dimensiones, de modo que la estructura geométrica sea visualizable.
#. Sobre ese plano reducido se ejecuta **K-Means** con *k = 3*; la hipótesis es que, si el espacio vectorial codifica inferencia, los clústeres tenderán a alinearse con las etiquetas originales (entailment, contradiction, neutral / true, false, uncertain).
#. Entrenamos un **árbol de decisión** limitado en principio a cuatro niveles: buscamos obtener una medida de cuán separables son las clases y, al mismo tiempo, revelar qué ejes del embedding concentran información lógica. 
#. Repetimos todo el proceso en FOLIO y comparamos patrones con SNLI. 
#. Adicionalmente, incorporariamos un *probing* específico: regresión logística dirigida a cuantificadores universales y existenciales para verificar si esos rasgos son recuperables directamente de los vectores. 
