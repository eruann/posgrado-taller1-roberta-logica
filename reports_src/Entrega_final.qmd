...
def safe_display(df, indexOption=False):
    ...
    # Aplicar escape a todas las columnas
    for col in df_clean.columns:
        df_clean[col] = df_clean[col].astype(str).apply(escape_latex)
    
    num_cols = len(df_clean.columns)
    
    # Generar tabla LaTeX simple
    latex_lines = []
    latex_lines.append("\\begin{table}[ht]")
    latex_lines.append("\\centering")
    
    # Usar tabular simple con columnas p{} para wrapping
...
def show_contamination_examples(df, overlap_texts, dataset_name):
    ...
    premise_cell = (
        f"<b>Premise:</b> \"{p_row[premise_col]}\"<br>"
        f"<b>{hypothesis_col.title()}:</b> \"{p_row[hypothesis_col]}\"<br>"
        f"<b>Label:</b> {p_row[label_col]}"
    )
    # Celda para "Aparece como Hipótesis"
    hypothesis_cell = (
        f"<b>Premise:</b> \"{h_row[premise_col]}\"<br>"
        f"<b>{hypothesis_col.title()}:</b> \"{h_row[hypothesis_col]}\"<br>"
        f"<b>Label:</b> {h_row[label_col]}"
    )
    ...
    # Convertimos el DataFrame a HTML, asegurándonos de que las etiquetas <br> no se escapen
    html = table_df.to_html(index=False, escape=False, justify='left')
    # Usamos la función de display para renderizar el HTML en Quarto
